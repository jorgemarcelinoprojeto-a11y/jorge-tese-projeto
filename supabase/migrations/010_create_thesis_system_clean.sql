-- ============================================================================
-- MIGRATION: Thesis System with Versioned Chapters (Clean Version)
-- Description: Create thesis-chapters system with versioning
-- Date: 2024-11-29
-- ============================================================================

-- ============================================================================
-- TABLE: theses
-- Purpose: Main container for thesis/dissertation projects
-- ============================================================================
...
-- ============================================================================
-- TABLE: chapters
-- Purpose: Chapters within a thesis, with ordering
-- ============================================================================
CREATE TABLE IF NOT EXISTS chapters (
  ...
  current_version_id UUID, -- Points to active version (set after creating version)
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),

  -- Ensure unique ordering within thesis
  UNIQUE(thesis_id, chapter_order),

  -- Ensure unique title within thesis
  UNIQUE(thesis_id, title)
);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_chapters_thesis_id ON chapters(thesis_id);
CREATE INDEX IF NOT EXISTS idx_chapters_order ON chapters(thesis_id, chapter_order);

-- ============================================================================
-- TABLE: chapter_versions
-- Purpose: Version history for each chapter
-- ============================================================================
CREATE TABLE IF NOT EXISTS chapter_versions (
  ...
  -- Provenance tracking
  created_by_operation TEXT NOT NULL DEFAULT 'upload', -- 'upload', 'improve', 'translate', 'adjust', 'adapt', 'update'
...
CREATE TABLE IF NOT EXISTS chapter_chunks (
  ...
  -- Ensure unique chunk index per version
  UNIQUE(chapter_version_id, chunk_index)
);

-- Indexes
CREATE INDEX IF NOT EXISTS idx_chapter_chunks_version_id ON chapter_chunks(chapter_version_id);
CREATE INDEX IF NOT EXISTS idx_chapter_chunks_chunk_index ON chapter_chunks(chapter_version_id, chunk_index);

-- Full-text search index
CREATE INDEX IF NOT EXISTS idx_chapter_chunks_text_search ON chapter_chunks USING gin(to_tsvector('portuguese', text));

-- ============================================================================
-- Foreign Key: chapters.current_version_id
-- Purpose: Add FK constraint for current_version_id after table creation
-- ============================================================================
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'fk_chapters_current_version'
  ) THEN
...
DROP POLICY IF EXISTS "Allow all access to theses" ON theses;
DROP POLICY IF EXISTS "Allow all access to chapters" ON chapters;
DROP POLICY IF EXISTS "Allow all access to chapter_versions" ON chapter_versions;
DROP POLICY IF EXISTS "Allow all access to chapter_chunks" ON chapter_chunks;
...
-- Function: Get next chapter order for a thesis
...
-- Function: Get next version number for a chapter
...
-- Function: Create new chapter version and update current_version_id
...
BEGIN
  ...
  -- Update chapter's current_version_id
  UPDATE chapters
...
-- View: Thesis with chapter count
...
-- View: Chapter with current version details
...
COMMENT ON COLUMN chapters.chapter_order IS 'Sequential ordering of chapters (1, 2, 3...). Can have gaps (e.g., 1, 3, 5 if chapter 2 not uploaded yet)';
COMMENT ON COLUMN chapters.current_version_id IS 'Points to the active/latest version of this chapter';
COMMENT ON COLUMN chapter_versions.parent_version_id IS 'Tracks version lineage (e.g., v3 was created from v2)';
COMMENT ON COLUMN chapter_versions.created_by_operation IS 'How this version was created: upload, improve, translate, adjust, adapt, update';
COMMENT ON COLUMN chapter_chunks.chunk_index IS 'Sequential position of chunk in document (0-based)';
