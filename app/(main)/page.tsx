...
import { FolderPlus, Folder, FileText, Sparkles, BookOpen, GraduationCap, Search, Trash2 } from 'lucide-react';
...
export default function HomePage() {
  ...
  const handleDeleteThesis = async (e: React.MouseEvent, thesisId: string, title: string) => {
    e.preventDefault();
    e.stopPropagation();
    if (!confirm(`Excluir a tese "${title}"? Todos os capítulos e versões serão removidos permanentemente.`)) return;
    try {
      setDeletingId(thesisId);
      const res = await fetch(`/api/theses/${thesisId}`, { method: 'DELETE' });
      if (!res.ok) throw new Error('Falha ao excluir tese');
                          ...
                          <CardHeader className="relative">
                            <div className="flex items-start justify-between mb-4">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  onClick={(e) => handleDeleteThesis(e, thesis.id, thesis.title)}
