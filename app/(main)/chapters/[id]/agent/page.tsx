...
import {
  ...
  CheckCircle2, AlertCircle, Bot, User as UserIcon
} from 'lucide-react';
...
export default function AgentModePage() {
  ...
  const pickModel = (preferred: 'openai' | 'gemini') => {
    const models = settings?.models?.[preferred];
    if (models && models.length > 0) return { provider: preferred, model: models[0] };
    // Fallback to anything available
    for (const p of ['gemini', 'openai', 'anthropic', 'grok'] as const) {
      const m = settings?.models?.[p];
      if (m && m.length > 0) return { provider: p, model: m[0] };
    }
    return null;
  };
    ...
    try {
      switch (cmd) {
        ...
        case '/traduzir': {
          ...
          const ai = pickModel('openai');
          if (!ai) { appendMessage({ role: 'system', content: 'Configure ao menos um modelo de IA em /settings.', status: 'error' }); return; }
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-white/10 bg-black/40 backdrop-blur-xl">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <Badge className="bg-red-500/15 text-red-400 border border-red-500/30 text-[10px] uppercase tracking-wider">Beta</Badge>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href={`/chapters/${chapterId}/versions/${selectedVersionId}`}>
            <Button variant="outline" size="sm" className="border-white/15 text-gray-300 hover:bg-white/10 text-xs">
              Modo clássico
            </Button>
          </Link>
        </div>
