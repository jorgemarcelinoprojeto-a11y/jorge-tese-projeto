...
/**
 * POST /api/adjust/[id]/apply - Apply accepted adjustments to document
 */
export async function POST(
  ...
  try {
    ...
    console.log(`[ADJUST-APPLY] Applying ${acceptedSuggestions.length} adjustments`);
    ...
    const tempOutputPath = path.join(tempDir, `${jobId}_${job.document_id}_adjusted.docx`);

    const buffer = Buffer.from(await fileBlob.arrayBuffer());
    await fs.writeFile(tempInputPath, buffer);

    // Apply adjustments using the same function as translations
    const suggestionsForDocx: ApplyDocxSuggestion[] = acceptedSuggestions.map((s: any) => ({
      id: s.id,
      originalText: s.originalText || '',
      improvedText: s.adjustedText || ''
    }));

    const applyResult = await applySuggestionsToDocx(tempInputPath, tempOutputPath, suggestionsForDocx);
    console.log('[ADJUST-APPLY] DOCX apply result:', applyResult);

    // Read adjusted file
    const adjustedBuffer = await fs.readFile(tempOutputPath);
    ...
    return new NextResponse(adjustedBuffer, {
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
        'Content-Disposition': `attachment; filename="${sanitizedTitle}_ajustado.docx"`
      }
    });
